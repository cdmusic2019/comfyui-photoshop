...
# ──────────────────────────────────────────────
async def _handle_workflow_complete(finished_prompt_id: str):
    """
    Wait 0.5 seconds to confirm if the task is truly stuck at the head of the queue.
    If it's a pure cache hit, forcibly remove it from the queue.
    """
    await asyncio.sleep(0.5)
    first = get_queue_first()
    if first and first.get("prompt_id") == finished_prompt_id:
        print(f"# PS: [Hook] Prompt {finished_prompt_id} finished without image output. Forcing queue cleanup for {first['client_id']}")
        await _complete_queue_head()
...
def _hooked_send_sync(event, data, sid=None):
    ...
    cleanup_events = {"execution_interrupted", "execution_error"}
    
    if event in cleanup_events:
        print(f"# PS: [Hook] ComfyUI event '{event}' detected, cleaning up queue...")
        try:
            asyncio.run_coroutine_threadsafe(
                _handle_generation_interrupted({"event": event, "data": data}, event), loop
            )
        except Exception as e:
            print(f"# PS: [Hook] Error handling {event}: {e}")
            
    # Bind ComfyUI's prompt_id to the head of the current queue.
    if event in ("execution_start", "execution_cached"):
        ...
        first = get_queue_first()
        if first and prompt_id:
            first["prompt_id"] = prompt_id

    # Accurately validate prompt_id when capturing the executing event and node is None.
    if event == "executing" and isinstance(data, dict) and data.get("node") is None:
        finished_prompt_id = data.get("prompt_id")
        first = get_queue_first()
        
        # If the completed prompt_id "=" the prompt_id at the head of the queue, trigger the check.
        if first and finished_prompt_id and first.get("prompt_id") == finished_prompt_id:
            try:
                asyncio.run_coroutine_threadsafe(_handle_workflow_complete(finished_prompt_id), loop)
            except Exception as e:
                print(f"# PS: [Hook] Error handling workflow complete: {e}")
...
# ============================================================
#  ★★★ 注册 ComfyUI 验证阶段 Hook（拦截节点加载/模型缺失等错误）★★★Model loading errors and other exceptions
# ============================================================
_original_validate_prompt = execution.validate_prompt

async def _hooked_validate_prompt(*args, **kwargs):
    valid = await _original_validate_prompt(*args, **kwargs)
    if not valid[0]:
        error_info = valid[1]
        global current_generating_client_id
        if current_generating_client_id:
            print(f"# PS: [ValidationHook] Validation failed! Target client: {current_generating_client_id}")
            mock_msg = {"validation_failed": error_info, "message": "Workflow Validation Failed."}
            try:
                asyncio.ensure_future(_handle_generation_interrupted(mock_msg, "validation_failed"))
            except Exception as e:
                print(f"# PS: [ValidationHook] Failed to execute error handler: {e}")
    return valid

execution.validate_prompt = _hooked_validate_prompt
print("# PS: Hooked into execution.validate_prompt for node/model missing detection")
